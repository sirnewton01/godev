...
define("godev/goTextStyler", [ //$NON-NLS-0$
	...
	var stringStyle = {styleClass: "cm-string"}; //$NON-NLS-0$
		...
		_default: function(c) {
			switch (c) {
				...
				default:
					...
					if ((97 <= c && c <= 122) || (65 <= c && c <= 90) || c === 95) { //LETTER OR UNDERSCORE OR NUMBER
						do {
							c = this._read();
						...
						if (keywords.length > 0) {
							...
							for (var i=0; i<keywords.length; i++) {
								if (this.keywords[i] === word) {
									// Imports have a special token type
									if (word === "import") {
										return IMPORT;
									}
									return KEYWORD;
	...
	function GoTextStyler (view, annotationModel) {
		this.commentStart = "/*"; //$NON-NLS-0$
		this.commentEnd = "*/"; //$NON-NLS-0$
		// All of the Go keywords from the language specification
		...
		// The beginning and end of the imports section
		this.importBegin = -1;
		this.importEnd = -1;
	...
	GoTextStyler.prototype = {
			...
			while ((token = scanner.nextToken())) {
				...
				switch (token) {
					case KEYWORD: 
						...
						if (this.importBegin !== -1 && 
							this.importEnd === -1 &&
							tokenStart > this.importBegin) {
							
							this.importEnd = tokenStart;
						}
						
						break;
					case IMPORT: 
						style = keywordStyle;
						// Set the beginning of the import section if it is not
						//  already set
						if (this.importBegin === -1 || this.importBegin > tokenStart) {
							this.importBegin = tokenStart;
						}
						break;
					case NUMBER: 
						style = numberStyle;
			...
			while ((token = scanner.nextToken())) {
				...
				switch (token) {
					...
					case WHITE_SPACE:
						...
						break;
					default:
						// The beginning of the import section must be established
						// The end of the import may not be known or this string
						//  must be before the established end of the import section.
						if (this.importBegin !== -1 && offset > this.importBegin && 
							(this.importEnd === -1 || offset < this.importEnd) && 
							this.detectHyperlinks) {
								
							style = this._detectImport(scanner.getData(), tokenStart, styles, stringStyle);
						}
				}
				if (style) {
					styles.push({start: tokenStart, end: scanner.getOffset() + offset, style: style});
		...
		_detectImport: function(text, offset, styles, s) {
			var href = null, index, linkStyle;
		...
		_onModelChanged: function(e) {
			...
			// Changes before the import begin
			if (this.importBegin !== -1 && this.importBegin >= start) {
				this.importBegin = this.importBegin - removedCharCount + addedCharCount;
			}
			
			// Changes before the import end
			if (this.importEnd !== -1 && this.importEnd >= start) {
				this.importEnd = this.importEnd - removedCharCount + addedCharCount;
			}
